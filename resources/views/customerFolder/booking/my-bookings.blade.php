    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .voucher-header::after {
            ...
            height: 5px;
            background: linear-gradient(90deg, #FFD709, #FFA500, #F59E0B);
        }
        ...
        .voucher-status-badge.pending   { background: rgba(255,215,0,0.15); color: #F59E0B; border-color: #FFA500; }
        .voucher-status-badge.confirmed { background: rgba(16,185,129,0.15); color: #10B981; border-color: #10B981; }
        .voucher-status-badge.completed { background: rgba(59,130,246,0.15); color: #3B82F6; border-color: #3B82F6; }
        .voucher-status-badge.cancelled { background: rgba(239,68,68,0.15); color: #EF4444; border-color: #EF4444; }
        .voucher-status-badge.refunded  { background: rgba(139,92,246,0.15); color: #8B5CF6; border-color: #8B5CF6; }
        ...
        .voucher-accom-row .v-name { font-size: 0.85rem; font-weight: 700; color: #1F2937; }
        .voucher-accom-row .v-type { font-size: 0.68rem; font-weight: 600; color: #6B7280; text-transform: uppercase; letter-spacing: 0.5px; margin-top: 0.15rem; }
        .voucher-accom-row .v-price { font-size: 0.95rem; font-weight: 800; color: #F59E0B; }
        ...
        .voucher-payment-row .v-ref { font-size: 0.82rem; font-weight: 700; color: #1F2937; }
        .voucher-payment-row .v-info { font-size: 0.68rem; color: #6B7280; font-weight: 600; margin-top: 0.15rem; }
        .voucher-payment-row .v-amt { font-size: 0.95rem; font-weight: 800; color: #10B981; }
        ...
        .voucher-price-line span:last-child { color: #1F2937; font-weight: 700; }

        .voucher-price-line.total {
        ...
        .voucher-price-line.total span:last-child {
            color: #10B981;
            font-size: 1.1rem;
            font-weight: 900;
        }
        ...
        /* Print voucher action bar (NOT printed) */
        .voucher-action-bar {
            display: flex;
        ...
        @media print {
            .voucher-action-bar { display: none !important; }
            .voucher-sheet { 
                border-radius: 0; 
                box-shadow: none; 
                max-width: 100%;
...
<body>
    ...
    <!-- Print Voucher Overlay -->
    <div id="print-voucher-overlay">
        <div class="voucher-sheet" id="voucher-sheet-content">
            <!-- filled by JS -->
        </div>
    </div>
...
document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeModal(); closePrintVoucher(); } });

/* ═══════════════════════════════════════════════════════════
   PRINT BOOKING
═══════════════════════════════════════════════════════════ */
function printBooking(id) {
    const overlay = document.getElementById('print-voucher-overlay');
    const sheet   = document.getElementById('voucher-sheet-content');

    sheet.innerHTML = `
        <div style="padding:3rem;text-align:center;">
            <div class="spinner" style="margin:0 auto 1rem;"></div>
            <p style="font-size:0.9rem;color:#6B7280;font-weight:600;">Preparing voucher...</p>
        </div>`;
    ...
    .then(r => { if (!r.ok) throw new Error('Failed to load booking'); return r.json(); })
    .then(d => { if (d.success) renderVoucher(d.booking); else throw new Error(d.message); })
    .catch(err => {
        sheet.innerHTML = `
            <div style="padding:3rem;text-align:center;">
                <div style="font-size:2rem;color:#EF4444;margin-bottom:1rem;">⚠️</div>
                <p style="font-weight:700;color:#1F2937;">Failed to load booking</p>
                <p style="font-size:0.85rem;color:#6B7280;margin-top:0.5rem;">${err.message}</p>
                <button onclick="closePrintVoucher()" class="btn btn-ghost" style="margin-top:1.5rem;"><i class="fas fa-times"></i> Close</button>
            </div>`;
    });
}
...
function renderVoucher(b) {
    ...
    const booked  = b.formatted_details?.created_at
        ? b.formatted_details.created_at.split(' ')[0]
        : (b.formatted_created_at ? b.formatted_created_at.split(' ')[0] : 'N/A');
    const printDate = new Date().toLocaleDateString('en-US', { year:'numeric', month:'long', day:'numeric', hour:'2-digit', minute:'2-digit' });

    // Accommodations
    let accomRows = '<p style="font-size:0.82rem;color:#9CA3AF;text-align:center;padding:0.8rem 0;">No accommodations listed.</p>';
    if (b.cart?.items?.length) {
    ...
    // Payments
    let payRows = '<p style="font-size:0.82rem;color:#9CA3AF;text-align:center;padding:0.8rem 0;">No payment records.</p>';
    if (b.payments?.length) {
        payRows = b.payments.map(p => {
            const date = new Date(p.paymentDate).toLocaleDateString('en-US', { year:'numeric', month:'short', day:'numeric' });
            return `
            <div class="voucher-payment-row">
                <div>
    ...
    const totalPrice    = b.formatted_details?.total_price || '₱0.00';
    const totalPaid     = b.formatted_details?.total_paid || '₱0.00';
    const totalBalance  = b.formatted_details?.remaining_balance || '₱0.00';

    sheet.innerHTML = `
        <!-- Header -->
        <div class="voucher-header">
            <div class="voucher-logo-row">
                <div>
                    <div class="voucher-brand">Villa Elena</div>
                    <div class="voucher-brand-sub">Booking Voucher</div>
                </div>
                <div class="voucher-badge-wrap">
                    <div class="voucher-status-badge ${status}">
                        <i class="fas ${badgeIcon(status)}"></i> ${cap(status)}
                    </div>
                </div>
            </div>
            <div class="voucher-title">Reservation Confirmation</div>
            <div class="voucher-booking-id">Booking #${b.bookingID}</div>
        </div>
        ...
        <div class="voucher-body">
            ...
            <div class="voucher-section">
                <div class="voucher-section-label"><i class="fas fa-info-circle"></i> Booking Information</div>
                <div class="voucher-info-grid">
                    <div class="voucher-info-item">
                        <div class="v-label">Check-in</div>
                        <div class="v-value">${start}</div>
                    </div>
                    <div class="voucher-info-item">
                        <div class="v-label">Check-out</div>
                        <div class="v-value">${end}</div>
                    </div>
                    <div class="voucher-info-item">
                        <div class="v-label">Guests</div>
                        <div class="v-value">${guests} guest${guests > 1 ? 's' : ''}</div>
                    </div>
                    <div class="voucher-info-item">
                        <div class="v-label">Event Type</div>
                        <div class="v-value">${b.eventType || 'Standard'}</div>
                    </div>
                    <div class="voucher-info-item">
                        <div class="v-label">Booked On</div>
                        <div class="v-value">${booked}</div>
                    </div>
                    <div class="voucher-info-item">
                        <div class="v-label">Status</div>
                        <div class="v-value">${cap(status)}</div>
                    </div>
                </div>
            </div>
            ...
            <!-- Accommodations -->
            <div class="voucher-section">
                <div class="voucher-section-label"><i class="fas fa-home"></i> Accommodations</div>
                ${accomRows}
            </div>

            <!-- Payment Summary -->
            <div class="voucher-section">
                <div class="voucher-section-label"><i class="fas fa-wallet"></i> Payment Summary</div>
                <div class="voucher-price-summary">
                    <div class="voucher-price-line"><span>Total Amount</span><span>${totalPrice}</span></div>
                    <div class="voucher-price-line"><span>Amount Paid</span><span style="color:#10B981">${totalPaid}</span></div>
                    <div class="voucher-price-line total"><span>Remaining Balance</span><span>${totalBalance}</span></div>
                </div>
            </div>

            <!-- Payment History -->
            <div class="voucher-section">
                <div class="voucher-section-label"><i class="fas fa-receipt"></i> Payment History</div>
                ${payRows}
            </div>

            ${b.specialRequirements ? `
            <div class="voucher-section">
                <div class="voucher-section-label"><i class="fas fa-sticky-note"></i> Special Requirements</div>
                <div class="voucher-special-req">
                    <p>${b.specialRequirements}</p>
                </div>
            </div>` : ''}
        ...
        <div class="voucher-footer">
            <div class="voucher-footer-note">Thank you for choosing Villa Elena. Please present this voucher upon arrival.</div>
            <div class="voucher-footer-date">Printed: ${printDate}</div>
        </div>

        <!-- Action Bar (hidden on print) -->
        <div class="voucher-action-bar">
            <button onclick="closePrintVoucher()" class="btn btn-ghost"><i class="fas fa-times"></i> Close</button>
            <button onclick="triggerPrint()" class="btn btn-print"><i class="fas fa-print"></i> Print / Save PDF</button>
        </div>
    `;
}
...
function closePrintVoucher() {
    document.getElementById('print-voucher-overlay').classList.remove('show');
    document.body.style.overflow = 'auto';
}
